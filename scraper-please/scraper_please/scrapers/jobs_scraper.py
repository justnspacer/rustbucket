"""
Job scraper with integrated filtering, matching, and cover letter generation.
"""
import os
import logging
import hashlib
...
class JobsScraper(BaseScraper):
    """
    Scraper for job postings with filtering, matching, and cover letter generation.
    ...
    def __init__(
        ...
        logger.info("JobsScraper initialized")
    ...
    def search_jobs(self, query: JobSearchQuery) -> List[JobPosting]:
        ...
        try:
            ...
            raise ScraperException(f"Job search failed: {e}")
    ...
    def extract_resume(self, file_path: str) -> Resume:
        ...
        Raises:
            ScraperException: If extraction fails
        """
        if not os.path.exists(file_path):
            raise ScraperException(f"Resume file not found: {file_path}")
        ...
        try:
            if file_ext == '.pdf':
                ...
                raise ScraperException(f"Unsupported file type: {file_ext}")
            ...
            raise ScraperException(f"Resume extraction failed: {e}")
    ...
    def generate_cover_letter(
        ...
        Raises:
            ScraperException: If generation fails or API key missing
        """
        if not self.openai_api_key:
            raise ScraperException("OpenAI API key not configured")
...
"""
            ...
            raise ScraperException(f"Cover letter generation failed: {e}")
