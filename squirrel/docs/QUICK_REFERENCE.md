...
venv_squirrel\Scripts\activate  # Windows
source venv_squirrel/bin/activate  # Linux/Mac
...
### Using SquirrelManager (Recommended)
...
with SquirrelManager() as manager:
    # Get squirrel
    squirrel = manager.get_squirrel(SquirrelType.FOOLSBALL)
...
### Using Squirrel Directly

```python
from squirrel import FoolsballSquirrel

squirrel = FoolsballSquirrel(
    source="espn",
    cache_enabled=True,
...
- SDK: `manager.switch_source(SquirrelType.FOOLSBALL, "nfl")`
...
Returns:
- Active squirrels count
- Active sources per squirrel type
...
├── squirrel/      # Core package
│   ├── squirrel_manager.py  # Centralized manager
│   ├── squirrels/        # Squirrel implementations
│   ├── models/          # Data models
...
1. **Use SquirrelManager** for most use cases - it handles lifecycle automatically
