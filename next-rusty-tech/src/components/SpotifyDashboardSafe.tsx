...
export default function SpotifyDashboardSafe() {
  ...
  const safeRender = (value: any, fallback: string = "Unknown"): string => {
    ...
    if (typeof value === 'object' && value.total !== undefined) return String(value.total);
    return fallback;
  };
      ...
      {profile && (
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-xl font-semibold">{safeRender(profile.spotify_data.display_name)}</h3>
              <p className="text-gray-600">
                Followers: {getFollowerCount(profile.spotify_data.followers)}
              </p>
              <p className="text-gray-600">Country: {safeRender(profile.spotify_data.country)}</p>
              <p className="text-gray-600">Product: {safeRender(profile.spotify_data.product)}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {currentlyPlaying && (
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-lg font-semibold">{safeRender(currentlyPlaying.item?.name)}</h3>
              <p className="text-gray-600">
                {currentlyPlaying.item?.artists?.map((artist: any) => safeRender(artist.name)).join(", ")}
              </p>
            </div>
          </div>
        </div>
            ...
            {topTracks.slice(0, 5).map((track, index) => (
              <div key={track.id || index} className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <p className="font-medium">{safeRender(track.name)}</p>
                  <p className="text-sm text-gray-600">
                    {track.artists?.map((artist: any) => safeRender(artist.name)).join(", ")}
                  </p>
                </div>
              </div>
            ))}
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="space-y-2">
            {playlists.slice(0, 5).map((playlist, index) => (
              <div key={playlist.id || index} className="flex items-center gap-3">
                {playlist.images?.[0] && (
                  <img 
                ...
                <div className="flex-1">
                  <p className="font-medium">{safeRender(playlist.name)}</p>
                  <p className="text-sm text-gray-600">
